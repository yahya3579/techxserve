...
const FloatingGeometry = () => {
  const shapes = [
    { type: 'circle', size: 60, color: 'bg-[var(--brand-primary)]/15', delay: 0 },
    { type: 'square', size: 40, color: 'bg-blue-500/20', delay: 2 },
    { type: 'triangle', size: 50, color: 'bg-purple-500/15', delay: 4 },
    { type: 'diamond', size: 35, color: 'bg-emerald-500/20', delay: 6 },
    { type: 'hexagon', size: 45, color: 'bg-orange-500/15', delay: 8 },
  ];
      ...
      {shapes.map((shape, index) => (
        <motion.div
          ...
          className={`absolute ${shape.color} ${
            ...
            shape.type === 'diamond' ? 'rounded-lg rotate-45' :
            'rounded-md'
          }`}
          style={{
            width: shape.size,
  ...
  return (
    <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 pointer-events-none">
      {orbitElements.map((element) => (
        ...
        >
          <motion.div
            className={`absolute ${element.color} rounded-full shadow-lg`}
            style={{
              width: element.size,
              height: element.size,
              top: 0,
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden pt-20 bg-white">
      ...
      <div className="absolute inset-0 pointer-events-none">
        {/* Floating rings */}
        <motion.div
          className="absolute top-1/3 left-1/4 w-40 h-40 border-2 border-[var(--brand-primary)]/20 rounded-full"
          animate={{
            rotate: [0, 360],
            scale: [1, 1.2, 1],
            opacity: [0.2, 0.6, 0.2],
        ...
        <motion.div
          className="absolute bottom-1/3 right-1/3 w-24 h-24 border border-blue-500/25 rounded-full"
          animate={{
            rotate: [360, 0],
            scale: [0.8, 1.4, 0.8],
            opacity: [0.3, 0.7, 0.3],
        ...
        {/* Animated lines */}
        <motion.div
          className="absolute top-1/2 left-1/6 w-32 h-0.5 bg-gradient-to-r from-purple-500/30 to-transparent"
          animate={{
            width: [0, 128, 0],
            opacity: [0, 1, 0],
            rotate: [0, 45, 90],
        ...
        <motion.div
          className="absolute bottom-1/4 left-2/3 w-24 h-0.5 bg-gradient-to-l from-emerald-500/30 to-transparent"
          animate={{
            width: [0, 96, 0],
            opacity: [0, 1, 0],
            rotate: [90, 135, 180],
