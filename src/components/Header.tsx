...
export default function Header({ currentPage, setCurrentPage }) {
  ...
  return (
    ...
    >
      ...
      <motion.div
        className="absolute inset-0 bg-gradient-to-r from-[var(--brand-primary)]/2 via-transparent to-blue-500/1"
        animate={{
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        >
          ...
          >
            ...
            {/* MEDIA Button next to logo */}
            <motion.button
              onClick={() => setCurrentPage('media')}
              initial={{ opacity: 0, x: -10 }}
              animate={{ opacity: 1, x: 0 }}
              transition={{ duration: 0.5, delay: 0.3 }}
              className={`ml-2 px-3 py-1.5 bg-black text-white rounded-lg font-medium transition-all duration-300 relative overflow-hidden group hover:scale-105 ${
